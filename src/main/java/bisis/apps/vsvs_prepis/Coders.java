...
//                return makeStatus(library);
            case CODER_TYPE_INTERNAL_MARK:
                return makeInternalMark(library, records);
    ...
    // from records
    public static List<Coder> makeInternalMark(String library, List<Record> records) {
        Map<String, String> map = new HashMap<>();

        for (Record record : records) {
            for (Primerak primerak : record.getPrimerci()) {
                map.put(primerak.getSigIntOznaka(), primerak.getSigIntOznaka());
            }
        }
