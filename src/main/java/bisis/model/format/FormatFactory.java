...
public class FormatFactory {
  ...
  public static UFormat getFormat(Connection conn) {
    ...
    Statement stmt;
    ResultSet rset;
    HashMap<String, UIndicator> firstIndicators = new HashMap<String, UIndicator>();
    HashMap<String, UIndicator> secondIndicators = new HashMap<String, UIndicator>();
    HashMap<Integer, UCoder> extCoderTypes = new HashMap<Integer, UCoder>();
    HashMap<String, UCoder> internalCoders = new HashMap<String, UCoder>();
    HashMap<String, UCoder> subsubfieldCoders = new HashMap<String, UCoder>();
    HashMap<String, List<USubsubfield>> subsubfields = new HashMap<String, List<USubsubfield>>();
    HashMap<String, List<USubfield>> subfields = new HashMap<String, List<USubfield>>();
    try {
      ...
      while (rset.next()) {
        ...
        String key = rset.getString(2) + rset.getString(1);
        List<USubsubfield> ssfList = subsubfields.get(key);
        if (ssfList == null) {
          ssfList = new ArrayList<USubsubfield>();
      ...
      while (rset.next()) {
        ...
        String key = rset.getString(1);
        List<USubfield> sfList = subfields.get(key);
        if (sfList == null) {
          sfList = new ArrayList<USubfield>();
        ...
        List<USubsubfield> ssfList = subsubfields.get(id);
        if (ssfList != null) {
          sf.setSubsubfields(ssfList);
          Iterator<USubsubfield> it = ssfList.iterator();
          while (it.hasNext()) {
            it.next().setOwner(sf);
          }
        }
      ...
      while (rset.next()) {
        ...
        List<USubfield> sfList = subfields.get(f.getName());
        if (sfList != null) {
          f.setSubfields(sfList);
          Iterator<USubfield> i = sfList.iterator();
          while (i.hasNext()) {
            i.next().setOwner(f);
          }
        }
