package bisis.format.serializers;

import java.util.HashMap;
import java.util.Iterator;

import bisis.format.FormatSerializer;
import bisis.format.UCoder;
import bisis.format.UField;
import bisis.format.UFormat;
import bisis.format.UIndicator;
import bisis.format.UItem;
import bisis.format.USubfield;
import bisis.format.USubsubfield;
...
public class XMLSerializer implements FormatSerializer {
  ...
  public String serializeFormat(UFormat fieldSet) {
    ...
    Iterator<UField> fields = fieldSet.getFields().iterator();
    while (fields.hasNext()) {
      UField field = fields.next();
      fieldToXML(retVal, field);
    }
    
    Iterator<UCoder> extCoders = externalCoders.values().iterator();
  ...
  private void fieldToXML(StringBuffer retVal, UField field) {
    ...
    Iterator<USubfield> subfields = field.getSubfields().iterator();
    while (subfields.hasNext()) {
      USubfield subfield = subfields.next();
  ...
  private void indicatorToXML(StringBuffer retVal, UIndicator ind, int index) {
    ...
    retVal.append("\">");
    retVal.append(LF);
    Iterator<UItem> items = ind.getValues().iterator();
    while (items.hasNext()) {
      UItem item = items.next();
  ...
  private void subfieldToXML(StringBuffer retVal, USubfield subfield) {
    ...
    if (subfield.getCoder() != null) {
      ...
      retVal.append(">");
      retVal.append(LF);
      Iterator<USubsubfield> subsubfields = subfield.getSubsubfields().iterator();
      while (subsubfields.hasNext()) {
        USubsubfield subsubfield = subsubfields.next();
  ...
  private void coderToXML(StringBuffer retVal, UCoder coder, String indent) {
    ...
    Iterator<UItem> items = coder.getItems().iterator();
    while (items.hasNext()) {
      UItem item = items.next();
      retVal.append(indent);
      retVal.append("  <item code=\"");
  ...
  private String LF;
  private HashMap<Integer, UCoder> externalCoders = new HashMap<Integer, UCoder>();
