...
public class HTMLSerializer implements FormatSerializer {
  ...
  public String serializeCoders(UFormat fieldSet) {
    ...
    StringBuffer buff = new StringBuffer(INITIAL_BUFFER_SIZE);
    List<UCoder> externalCoders = new ArrayList<UCoder>();
    for (int i = 0; i < fieldSet.getFields().size(); i++) {
      UField field = (UField)fieldSet.getFields().get(i);
    ...
    Iterator<UCoder> it = externalCoders.iterator();
    while (it.hasNext()) {
      UCoder coder = it.next();
      retVal.append(lineSep);
  ...
  private void serializeCoders(StringBuffer buff, USubfield sf, List<UCoder> extCoders){
  ...
  private void serializeCoders(StringBuffer buff, USubsubfield ssf, List<UCoder> extCoders) {
  ...
  private void writeCoderItems(StringBuffer buff, UCoder coder) {
    ...
    buff.append(lineSep);
    Iterator<UItem> it = coder.getItems().iterator();
    while (it.hasNext()) {
      UItem item = it.next();
